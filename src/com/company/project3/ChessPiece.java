...
public abstract class ChessPiece implements IChessPiece {
	...
	public boolean isValidMove(Move move, IChessPiece[][] board) {
		...
		if (!((move.fromRow == move.toRow) && (move.fromColumn == move.toColumn))) { //If its new location is not the original location
			...
			//{
				if (board[move.toRow][move.toColumn] == null || board[move.toRow][move.toColumn].player() != board[move.toRow][move.toColumn].player()) //Can only move into a space with no piece or a enemy piece
				{
					valid = true;
				}
			//}
